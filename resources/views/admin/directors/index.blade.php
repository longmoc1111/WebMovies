...
@section("admin")
    ...
    <main class="main">
                ...
                <div class="col-12">
                    <div class="main__title">
                        ...
                        <div class="main__title-wrap">
                            ...
                            <form action="{{route("admin.director.sort")}}" class="filter__select">
                                <select name="sort" class="filter__select" name="sort" id="filter__sort"
                                    onchange="this.form.submit()">
                                    <option value="Tên" {{request("sort") == "Tên" ? "selected" : "" }}>Tên đạo diễn</option>
        ...
        <!-- modal-delete -->
        @foreach ($Directors as $director)
            <div class="modal fade" id="{{$director->DirectorID}}" tabindex="-1" aria-labelledby="modal-delete"
                aria-hidden="true">
                <div class="modal-dialog modal-dialog-centered">
                    <div class="modal-content">
                        <div class="modal__content">
                            <form action="{{route("admin.director.destroy", $director->DirectorID)}}" method="POST"
                                class="modal__form">
                                @csrf
                                @method("DELETE")
...
@section("footeradmin")
    @if (session("addDirector"))
        <script>
            iziToast.success({
                message: "{{session("addDirector")}}",
                position: "topRight"
            })
        </script>
    @endif

    @if(session("editDirector"))
        <script>
            iziToast.success({
                message: "{{session("editDirector")}}",
                position: "topRight"
            })
        </script>
    @endif

    @if (session("deleteDirector"))
        <script>
            iziToast.success({
                message: "{{session("deleteDirector")}}",
                position: "topRight"
            })
        </script>
